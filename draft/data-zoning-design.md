...
Data Zones define how data is organized, stored, and accessed in alignment with
...
The Data Zone applies to any [Data Domain](../data-domains/not-implement-yet.md) Google project with IaC.

---

## Architecture Model

The platform supports both **Lambda** and **Kappa** architectures:
...
| --- | --- | --- | --- | --- |
| **Raw** | Exact copy of source data | Data Engineering | 60 days (then archive) | Hourly |
| **Bronze** | Validated & standardized schema | Data Engineering | Indefinite | Hourly |
| **Silver** | Business-modeled data | Analytics Engineering | 3 years | Hourly |
| **Gold** | Analytics-ready data marts | Analytics Engineering | Varies by mart | Hourly |
| **Staging** | Temporary working tables | Data Engineering / AE | Short-lived | — |
| **Quarantine** | Failed quality-check records | Data Engineering | TBD | — |
| **Shared** | Cloned/shared trusted data | Shared (DE + AE) | Inherited from source | Hourly |
| **Access** | Row/column-level secured views | Shared (DE + AE) | N/A (views only) | Hourly |
...
> DE and AE access the underlying objects in each zone directly. End users must go through the **Access zone** only.
...
> All sensitive data is encrypted or tokenized before leaving the Raw zone. Downstream zones (Bronze onward) never store plain PII or card data.

**Storage:**

- **Cloud Storage**
  - File pattern: `source/year={yyyy}/month={mm}/day={dd}/hour={hh}`
  - Supported formats: `NEWLINE_JSON`, `JSON`, `CSV`, `Excel`, `AVRO`, `Parquet`
...
- **BigQuery**
  ...
  - Lambda hot tables use time-window suffixes on table names:

    | Suffix | Window |
    | --- | --- |
    | `{table}_curr_hour` | Current hour |
    | `{table}_curr_day` | Current day |
    | `{table}_curr_month` | Current month |

**Retention:** 60 days active → archive | **SLA:** Hourly
...
| `event_timestamp` | Original source event time | Business time used for all downstream aggregations |
...
**Retention:** Indefinite | **SLA:** Hourly
...
**Retention:** 3 years | **SLA:** Hourly (sub-5 min for POS and Inventory domains — see Retail Domain Reference)
...
| **Report** | `rpt_` | Pre-aggregated summary datasets |
...
**Retention:** Varies by mart | **SLA:** Hourly
...
> These zones exist in **BigQuery only** and serve access control, error handling, and data sharing needs.
...
This zone is the secure interface between the data platform and end users. All objects in Silver, Gold, and Shared are exposed exclusively through this layer with appropriate permissions applied.
...
Dataplex serves as the unified metadata and discovery layer across all zones. It spans both Cloud Storage (Iceberg tables) and BigQuery (native tables, BigLake external tables, views).

| Capability | Detail |
| --- | --- |
| **Discovery** | Automatic scan of GCS and BigQuery; tables registered in Dataplex catalog |
| **Data quality** | Dataplex quality rules complement pipeline-level checks in Bronze and Silver |
| **Lineage** | Zone-to-zone data lineage tracked for auditing and impact analysis |
| **Access** | Dataplex respects BigQuery IAM and Access zone RLS/CLS — catalog visibility does not grant data access |

> Analysts and AEs use Dataplex to find tables across zones. Browsing the catalog does not bypass Access zone controls.
...
End users who require data access must submit an access request. Access is provisioned through the Access zone exclusively — no direct zone access is granted to end users.
